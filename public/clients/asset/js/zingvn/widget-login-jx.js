  ...
  ExcuteValidLogin: function (zid, acn, obj, linkDownload) {
    if (zid > 0) {
      ...
      if (obj["logged"] === undefined) {
        ...
        if (linkdirect.split("zing").length > 1) {
          // check domain .zing.vn
        ...
        htmlA =
          '<a href="https://sso3.zing.vn/logout?return=' +
          linkdirect +
          apiPro +
          '"' +
        ...
        if (typeof acn != undefined) {
          jQuery(".content-alert-bregis > h3").append(
            'Bạn hiện đang đăng nhập với tài khoản : <span class="imp"> ' +
              acn +
              "</span>"
          );
        }
